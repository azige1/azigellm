...
from codeharvest.analysis.slicing.statements import StatementNode

class SliceOutputMode(Enum):
    MARKDOWN_FILES = "markdown_files"
    PATH_COMMENT = "path_comment"

class StatementDependencyEdge:
    def __init__(self, vertex1: StatementNode, vertex2: StatementNode, symbol: str):
...
class StatementDependencyGraph:
    def __init__(self, inputstmts: list[StatementNode]):
        self._adjacency_list: dict[StatementNode, list[StatementDependencyEdge]] = (
            defaultdict(list)
        )
    ...
    def add_edge(
        self, vertex1: StatementNode, vertex2: StatementNode, symbol: str
    ) -> None:
        edge = StatementDependencyEdge(vertex1, vertex2, symbol)
    ...
    def get_edges_for_vertex(self, vertex) -> list[StatementDependencyEdge]:
        return self._adjacency_list[vertex]

    def get_vertices(self) -> list[StatementNode]:
    ...
    def topological_sort(self) -> list[tuple[int, str, str]]:
        ...
        def dfs(vertex: StatementNode, is_root=False):
            visited.add(vertex)
    ...
    def unparse(
        self,
        unparse_type: SliceOutputMode = SliceOutputMode.PATH_COMMENT,
    ) -> str:
        ...
        if len(self._adjacency_list) == 0 and len(self.file_path_adjacency_list) == 0:
            if unparse_type == SliceOutputMode.MARKDOWN_FILES:
                output += f"```python\n"
