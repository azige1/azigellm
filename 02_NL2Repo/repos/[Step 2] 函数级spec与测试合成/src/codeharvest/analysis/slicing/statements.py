...
from codeharvest.core import FileRecord
from codeharvest.analysis.imports import ImportRewriter
from codeharvest.analysis.ast_tools import parse_ast, unparse_stmt_keep_comments
...
class StatementNode:
    ...
    def unparse_stmt(self, with_comments=True) -> str:
        if with_comments:
            return unparse_stmt_keep_comments(self.file.file_content, self.stmt)
...
class StatementTable:
    ...
    def create_fake_import_aststmt(
        ...
        return StatementNode(import_stmt, -1, self.file, import_stmt, -1)
    ...
    def transform_imports(
        ...
        ImportRewriter.rewrite_import_node(file_path, transformed_stmt)  
    ...
    def transform_stmt(self, stmt: ast.stmt) -> list[ast.stmt]:
        if isinstance(stmt, (ast.Import, ast.ImportFrom)):
            transformed_imports = StatementTable.transform_imports(self.file_path, stmt)
    ...
    def build_statements_list(self) -> list[StatementNode]:
        file_ast = parse_ast(self.file.file_content)
        ...
        for idx, stmt_obj in enumerate(self.statements_list):
            ...
            if isinstance(stmt, ast.Assign):
                for target in stmt.targets:
                    assigned_targets = StatementTable.assigned_expr_name_str(target)
                ...
                assigned_target = StatementTable.assigned_expr_name_str(stmt.target)
    ...
    def assigned_expr_name_str(expr: ast.expr) -> list[str] | None:
        if isinstance(expr, ast.Name):
            ...
            return StatementTable.assigned_expr_name_str(expr.value)
        elif isinstance(expr, ast.Subscript):
            return StatementTable.assigned_expr_name_str(expr.value)
        elif isinstance(expr, (ast.Tuple, ast.List)):
            assigned_names = []
            for el in expr.elts:
                assigned_name = StatementTable.assigned_expr_name_str(el)
    ...
    def resolve_wildcard_imports(self) -> list[StatementNode]:
        resolved: list[StatementNode] = []
        for idx in self.wildcard_idxs:
            resolved.append(self.statements_list[idx])
        return resolved

    def find_function_stmt_with_name(self, function_name: str) -> StatementNode | None:
    ...
    def find_class_stmt_with_name(self, class_name: str) -> StatementNode | None:
        for stmt_obj in self.statements_list:
            stmt = stmt_obj.stmt
